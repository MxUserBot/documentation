...
Universal send. Supports text, HTML, media, editing, keyboards, custom emoji.
...
If `edit_id` is not specified but the event is from the bot — it edits its own message.
...
**Don't rely** on auto-reply grabbing — if you need the text of the message the user replied to, use `get_reply_event` / `get_reply_text` manually.
...
The event being replied to. Decrypts, looks up edits.
...
Usually easier via `utils.answer(mx, media=...)`.
...
Auto: encryption, thumbnail for images, dimension detection.
...
See [Emoji Callbacks](emoji-callbacks.md).
