...
```
my_module.zip
└── my_module/                  <- Корневая папка = имя модуля
    ├── __init__.py             <- Meta + class Module (обязательно)
    ├── helpers.py              <- Вспомогательные функции
    ├── models.py               <- Pydantic модели
    └── services/
        ├── __init__.py
        └── api.py              <- Логика API вызовов
```
...
- `class Meta` должен быть **в `__init__.py`**, не внутри класса Module
- Имя корневой папки в ZIP = имя модуля
- Все вспомогательные `.py` файлы импортятся рекурсивно
- Если в папке есть другие папки — они тоже импортируются (как submodules)
- `__init__.py` не обязан содержать класс Module — он может быть в любом файле пакета
