...
```
Если аргумент со значением `None` остался `None` (пользователь не передал) — бот покажет хелп команды.

Например `.tr` без аргументов: `lang="en"` (есть дефолт), `text=None` (нет аргумента) → показать хелп.

Но если пользователь ответил на чьё-то сообщение своим `.tr` — бот может взять текст из реплая вместо аргумента. Это работает автоматически.
